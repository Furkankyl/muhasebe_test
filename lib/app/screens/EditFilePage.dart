...
class _EditFilePageState extends State<EditFilePage> {
  ...
  @override
  void initState() {
    date = widget.fileModel.date;
    fileNameController.text = widget.fileModel.name.split(".").first;
    _file = File.fromUri(Uri.parse("widget.fileModel.url"));
  ...
  editFile() async {
    ...
    FileModel fileModel = FileModel(
        ...
        url: fileIsChanging
            ? await uploadFile((await db.getCurrentUser()).id)
            : widget.fileModel.url,
        date: date,
        editingDate: DateTime.now());

    bool result = await db.fileUpdate(fileModel);
    if (result) {
      setState(() {
        isComplete = true;
      });
            ...
            : SafeArea(
                child: ListView(
                  ...
                  children: <Widget>[
                    ...
                    CustomButton(
                      child: Text('Kaydet'),
                      onPressed: isValid ? editFile : null,
                    )
                  ],
                ),
              ),
